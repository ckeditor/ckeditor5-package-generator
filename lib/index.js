	...
	for ( const templatePath of templatesToCopy ) {
		...
		if ( TEMPLATES_TO_FILL.includes( templatePath ) ) {
			data = {
				...
				dllWindowLibrary: dllConfiguration.windowLibrary
			};
		}
...
function getDllConfiguration( directory ) {
	if ( directory.startsWith( '@' ) ) {
		...
		const [ scope, packageName ] = directory.split( '/' );
		...
		return {
			library: `[ '${ scope.slice( 1 ) }', '${ packageNameSlug }' ]`,
			windowLibrary: `${ scope.slice( 1 ) }.${ packageNameSlug }`,
