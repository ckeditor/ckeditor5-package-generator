...
describe( 'lib/utils/validate-package-name', () => {
	...
	it( 'shows a note and prompts when the package name is missing', async () => {
		const validatedPackageName = await validatePackageName( undefined );
	...
	it( 'shows a note and prompts when the provided package name is invalid', async () => {
		await validatePackageName( 'foo-bar' );
	...
	it( 'shows a specific validation reason when the provided package name contains capital letters', async () => {
		await validatePackageName( 'ckeditor5-Foo' );

		expect( showNote ).toHaveBeenCalledWith(
			expect.stringContaining( 'The package name cannot contain capital letters.' ),
			'Package name'
		);
		expect( promptText ).toHaveBeenCalledWith( expect.objectContaining( {
			initialValue: 'ckeditor5-Foo'
		} ) );
	} );

	it( 'does not prompt when the provided scoped package name is valid', async () => {
		const validatedPackageName = await validatePackageName( '@ckeditor/ckeditor5-foo' );
	...
	it( 'does not prompt when the provided unscoped package name is valid', async () => {
		const validatedPackageName = await validatePackageName( 'ckeditor5-foo' );
	...
	describe( 'prompt validator', () => {
		it( 'accepts both scoped and unscoped package names', async () => {
			await validatePackageName( undefined );
		...
		it( 'rejects empty values', async () => {
			...
			expect( validator( '' ) ).toEqual( 'Choose a package name to get started.' );
		} );
		...
		it( 'rejects non-allowed characters', async () => {
			...
			expect( validator( 'ckeditor5-fóo' ) ).toEqual( 'The package name contains non-allowed characters.' );
		} );
	} );
} );
